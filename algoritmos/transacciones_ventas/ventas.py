...
def mostrar_ventas(ventana, volver_callback):
    ...
    def guardar_venta():
        ...
        for item in productos_en_venta:
            ...
            actualizar_stock(item["codigo_producto"], item["cantidad"], sumar=False)
    ...
    # Botón centrado bajo todo el formulario
    tk.Button(frame_form, text="Agregar Producto", bg="#2196F3", fg="white",
              font=("Arial", 11, "bold"), command=agregar_producto).grid(row=3, column=0, columnspan=2, pady=10)
    ...
    tk.Button(frame_izq, text="Registrar Venta", bg="#4CAF50", fg="white",
              font=("Arial", 12, "bold"), width=30, command=guardar_venta).pack(pady=8)
    ...
    # Contenedor centrado internamente
    buscar_contenedor = tk.Frame(frame_buscar, bg="#F0F0F0")
    buscar_contenedor.pack(anchor="center")

    tk.Label(buscar_contenedor, text="Buscar:", font=("Arial", 12), bg="#F0F0F0").pack(side="left", padx=5)
    entry_buscar = tk.Entry(buscar_contenedor, width=40, justify="center")
    ...
    tk.Button(frame_der, text="Anular Venta", bg="#E04E4E", fg="white",
              font=("Arial", 12, "bold"), command=anular_venta).pack(pady=8)
